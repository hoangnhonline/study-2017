...
class QuizAnswersController extends Controller
{
    ...
    */
    ...
    {   
        ...
        $content = isset($request->content) && $request->content != '' ? $request->content : '';

        $items =QuizQuestions::where('question_id', $question_id)->orderBy('display_order')->paginate(40);
        
        $cateArr = QuizCate::getList(['limit' => 100]);  

        $quizDetail = Quiz::find($question_id);        
        $quizList = Quiz::getList(['cate_id' => $quizDetail->cate_id, 'limit' => 1000]);
        return view('backend.quiz-questions.index', compact( 'items', 'cateArr' , 'content', 'question_id', 'quizDetail', 'quizList'));
    }
    ...
    */
    public function create(Request $request)
    {
        ...
        $quizDetail = Quiz::find($question_id);  
        $quizList = Quiz::getList(['cate_id' => $quizDetail->cate_id, 'limit' => 1000]);
        return view('backend.quiz-questions.create', compact( 'cateArr', 'question_id', 'quizDetail', 'quizList'));
    }
    ...
    */
    public function store(Request $request)
    {
        ...
        return redirect()->route('quiz-questions.index', ['question_id' => $dataArr['question_id']]);
    }
