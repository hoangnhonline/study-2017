...
<div class="block block-breadcrumb">
	<ol class="breadcrumb">
		...
		<li><a href="{!! route('thpt-cate', $classDetail->slug) !!}" title="{{ $classDetail->name }}">{{ $classDetail->name }}</a></li>			
		<li class="active">{!! $subjectDetail->name !!}</li>
	</ol>
</div><!-- /block-breadcrumb -->
<div class="block-list-course block-title-cm">
	<div class="block-title">	
		<h2 class="title">
			@if(isset($subjectStemDetail))
				{{ $subjectStemDetail->name }} - {!! $subjectDetail->name !!}
			@else
			 
			 {!! $subjectDetail->name !!} - {{ $classDetail->name }}
			 @endif
			</h2>		
	</div><!-- /block-title -->
	<div class="block-content">		
		...
		<p style="margin-bottom: 15px;">
			...
			<select class="form-control" id="subject_id">
				...
				<option value="{{ $sub->slug }}" {{ isset($subject_id) && $subject_id == $sub->id ? "selected" : "" }}>{!! $sub->name !!}</option>
				@endforeach
			</select>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#subject_id').change(function(){		
			if($(this).val() != ''){
				location.href = "{{ env('APP_URL') }}/giao-duc-thpt/{{ $slugClass }}/{{ $slugClassStem }}?m=" + $(this).val() ; 	
			}else{
				location.href = "{{ env('APP_URL') }}/giao-duc-thpt/{{ $slugClass }}/{{ $slugClassStem }}?m="; 	
			}
